...
The ratio of (n-1) to n is to correct for using samples of (n-1) instead of n, and the second factor of (n-1) is to account for the fact that we calculated a standard deviation of (n-1) samples instead of a sample of 1.
...
mean.jackknife <- function(a_vector) {
  ...
  for(i in 1:n){
    theta[i] = mean(a_vector[-i])
  }
  jackknife.stderr = sqrt(var(theta)*(n-1)^2/n)
...
gamma.jackknife <- function(a_vector) {
  ...
  s_vec = vector(mode="numeric",length=n)
  a_vec = vector(mode="numeric",length=n)
  for(i in 1:n) {
    s_vec[i] = gamma.est(a_vector[-i])[["s"]]
    a_vec[i] = gamma.est(a_vector[-i])[["a"]]
  }
    s.var = var(s_vec)*(((n-1)^2)/n)
    a.var = var(a_vec)*(((n-1)^2)/n)
  
jackknife.stderrs <- c(se.a.est = sqrt(a.var), se.s.est=sqrt(s.var))
...
jackknife.lm <- function(df,formula,p) {
  ...
  coeffs = matrix(nrow=n, ncol=p)
  for(i in 1:n) {
    model = lm(formula,data=df[-i,])
    for(j in 1:p){
      coeffs[i,j] = model$coefficients[j]
    }
  }
  jackknife.vars = (n-1)^2/n*apply(coeffs, 2, var)
...
- Write a function `omit_and_est` that takes the data with an omitted point and returns whatever function your estimator does.  
    ```{r,eval=T}
        ...
        omit_and_est <- function(omit, estimator, the_data) { 
          omitted = omit.case(the_data, omit)
          estimator(omitted)
        }
...
jackknife <- function(estimator,the_data) {
  ...
  n = ifelse(is.vector(the_data), length(the_data), dim(the_data)[1])
  
  
  
  jackknife.ests <- matrix(sapply(1:n, omit_and_est, estimator=estimator, the_data=the_data), ncol=n)
