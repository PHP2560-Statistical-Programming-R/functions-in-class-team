...
```{r, eval=FALSE}
  ...
  myTable = (data.frame(matrix(NA, nrow = nrow(df), ncol = p)))
  
  for (a in 1:p){
    for (b in 1:nrow(df)){
        new_lm = lm(formula, df[-b, a])
        myTable[b, a] = new_lm[[1]]
    }}

  for (c in 1:p){
    jacknife.stderr = sqrt(var(myTable[,c]) * (n-1)^2 / n)
    return(jacknife.stderr)
  }}
